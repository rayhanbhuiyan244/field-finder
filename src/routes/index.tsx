...
const facilities = [
  ...
  { icon: ShoppingBag, title: "Equipment Rental", body: "Bibs, balls, cones and goalkeeper gloves." },
...
function Landing() {
  ...
  useEffect(() => {
    listReviews().then(setReviews).catch(() => setReviews([]));
    listPricingRules().then(setPricingRules).catch(() => setPricingRules([]));
    getBusinessSettings().then((b) => { if (b) setBusiness(b); }).catch(() => {});
  }, []);
    ...
    <PublicLayout>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto flex min-h-[86vh] w-full max-w-7xl flex-col justify-center px-6 py-24 text-primary-foreground">
          ...
          <p className="mt-6 max-w-xl text-lg text-primary-foreground/85">
            Premium 5-a-side turf, floodlights on till midnight, and instant online booking.
            Pick your slot, invite the squad, and hit the pitch.
          </p>
          ...
          <div className="mt-12 grid max-w-3xl grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <div key={x.l} className="rounded-xl border border-white/15 bg-white/5 p-3 backdrop-blur">
      ...
      <section className="mx-auto -mt-16 w-full max-w-7xl px-6 relative z-10">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard label="Available Slots Today" value={stats.availableToday} icon={Calendar} tone="accent" trend="Updated live" />
          <StatCard label="Matches Played" value={stats.matchesPlayed.toLocaleString()} icon={Trophy} tone="primary" />
          <StatCard label="Happy Players" value={`${(stats.happyPlayers / 1000).toFixed(1)}K+`} icon={Users} tone="secondary" />
          <StatCard label="Average Rating" value={`${stats.averageRating} / 5`} icon={Star} tone="warning" trend="Based on 620 reviews" />
        </div>
      </section>

      {/* Facilities */}
      <section className="mx-auto w-full max-w-7xl px-6 py-24">
        <SectionHeader eyebrow="Facilities" title="Everything you need on match day" subtitle="A modern arena built for players — not just fields with fences." />
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {facilities.map((f) => (
            <div key={f.title} className="rounded-2xl border border-border/60 bg-card p-6 shadow-[0_2px_8px_-4px_rgba(15,23,42,0.06)] hover:-translate-y-0.5 hover:shadow-[0_12px_32px_-16px_rgba(15,23,42,0.2)] transition-all">
      ...
      <section id="gallery" className="mx-auto w-full max-w-7xl px-6 pb-24">
        ...
        <div className="mt-12 grid grid-cols-2 gap-3 md:grid-cols-4">
          <img src={g1} alt="Match under floodlights" loading="lazy" className="col-span-2 row-span-2 h-full w-full rounded-2xl object-cover aspect-square" />
          <img src={g2} alt="Football on turf" loading="lazy" className="h-full w-full rounded-2xl object-cover aspect-square" />
          <img src={g3} alt="Aerial view" loading="lazy" className="h-full w-full rounded-2xl object-cover aspect-square" />
          <img src={g4} alt="Players celebrating" loading="lazy" className="h-full w-full rounded-2xl object-cover aspect-square" />
          <img src={g5} alt="Changing room" loading="lazy" className="h-full w-full rounded-2xl object-cover aspect-square" />
          <img src={g6} alt="Goal net" loading="lazy" className="col-span-2 h-full w-full rounded-2xl object-cover aspect-square md:aspect-auto" />
        </div>
      </section>
      ...
      <section className="bg-muted/40 border-y border-border/60 py-24">
        <div className="mx-auto w-full max-w-7xl px-6">
          <SectionHeader eyebrow="Pricing" title="Transparent hourly pricing" subtitle="No hidden fees. Pay only for the time you play." />
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {pricingRules.map((p, i) => (
              <div key={p.id} className={`relative rounded-2xl border p-6 shadow-[0_2px_8px_-4px_rgba(15,23,42,0.06)] ${i === 2 ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border/60"}`}>
                ...
                <div className={`mt-6 flex items-center gap-2 text-sm ${i === 2 ? "text-accent" : "text-secondary"}`}>
      ...
      <section className="mx-auto w-full max-w-7xl px-6 py-24">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {reviews.map((r) => (
            <div key={r.id} className="rounded-2xl border border-border/60 bg-card p-6 shadow-[0_2px_8px_-4px_rgba(15,23,42,0.06)]">
              <div className="flex items-center gap-3">
                <div className="grid h-11 w-11 place-items-center rounded-full bg-primary/10 text-primary font-semibold">{r.initials}</div>
                <div>
          ...
          <div className="lg:col-span-2 overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm">
            <div className="relative aspect-[16/9] w-full bg-gradient-to-br from-secondary/30 via-primary/10 to-accent/20">
              ...
              <div className="absolute bottom-4 left-4 rounded-xl bg-card px-4 py-2 shadow-lg">
                ...
                <p className="text-sm font-semibold">Indiranagar, Bengaluru</p>
              </div>
            </div>
          </div>
...
function SectionHeader({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle?: string }) {
  return (
    <div className="mx-auto max-w-2xl text-center">
      <span className="inline-block rounded-full bg-secondary/15 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-secondary">{eyebrow}</span>
