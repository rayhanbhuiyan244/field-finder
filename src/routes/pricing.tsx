...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Transparent hourly pricing for weekday, weekend, peak, and night slots.",
      },
    ],
  }),
  component: Pricing,
      ...
      <div className="mx-auto w-full max-w-7xl px-6 py-16">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            Pay only for the hour you play. Cancel up to 6 hours before start.
          </p>
        </div>
        ...
        {!loading && error && (
          <div className="mt-12">
            <ErrorState variant="network" onRetry={refresh} />
          </div>
        )}
        {!loading && !error && pricingRules.length === 0 && (
          <div className="mt-12">
            <EmptyState
              ...
              description="We're finalising rates. Check back shortly."
            />
          </div>
        )}
        {!loading && !error && pricingRules.length > 0 && (
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {pricingRules.map((p, i) => (
              ...
              >
                ...
                <ul
                  className={`mt-6 space-y-2 text-sm ${i === 2 ? "text-primary-foreground/90" : "text-foreground"}`}
                >
                  <li className="flex items-center gap-2">
                    <CheckCircle2
                      className={`h-4 w-4 ${i === 2 ? "text-accent" : "text-secondary"}`}
                    />{" "}
